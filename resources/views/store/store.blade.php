                    ...
                    <div class="card product-card-alt">
                        <div class="product-thumb">
                            <div class="product-card-actions"><a class="btn btn-light btn-icon btn-shadow fs-base mx-2"
                                    href="{{ route('products') . '/' . $anProduct->id }}""><i class="icon-eye-open"></i></a>
                                <form action="{{ route('cart') }}" method="post" class="fs-base mx-2"
                                    style="display: inline">
                                    @csrf
                                    <input type="hidden" name="product_id" value="{{ $anProduct->id }}">
                                    <button class="btn btn-light btn-icon btn-shadow" alt="add cart"><i
                                            class="icon-shopping-cart"></i></button>
                                </form>
                            </div><a class="product-thumb-overlay"
                                href="{{ route('products') . '/' . $anProduct->id }}"></a><img
                                src="{{ url('img/products/01.jpg') }}" alt="Product">
                        </div>
                        <div class="card-body">
                            <div class="d-flex flex-wrap justify-content-between align-items-start pb-2">
                                <div class="text-muted fs-xs me-1">{{ $anProduct->name }}</a></div>
                                <div class="star-rating"><i class="icon-star active"></i><i class="icon-star active"></i><i
                                        class="icon-star active"></i><i class="icon-star active"></i><i
                                        class="icon-star active"></i>
                                </div>
                            </div>
                            <div class="d-flex flex-wrap justify-content-between align-items-center">
                                ...
                                <div class="bg-faded-accent text-accent rounded-1 py-1 px-2">
                                    ${{ number_format($anProduct->price, 2) }}</div>
                            </div>
                        </div>
                    </div>
                </div>
